...
def flashlight_stow
  ...
  cmd("DRONE DO_SET_SERVO with PARAM1 #{FLASHLIGHT_SERVO_PORT}, PARAM2 #{FLASHLIGHT_STOW_PWM}")
  puts "Flashlight stowed (PWM #{FLASHLIGHT_STOW_PWM})."
end

def flashlight_deploy
  puts "--- Deploying flashlight ---"
  cmd("DRONE DO_SET_SERVO with PARAM1 #{FLASHLIGHT_SERVO_PORT}, PARAM2 #{FLASHLIGHT_DEPLOY_PWM}")
  puts "Flashlight deployed (PWM #{FLASHLIGHT_DEPLOY_PWM})."
end

def flashlight_on
  puts "--- Turning flashlight ON ---"
  cmd("DRONE DO_SET_SERVO with PARAM1 #{FLASHLIGHT_POWER_PORT}, PARAM2 #{FLASHLIGHT_ON_PWM}")
  puts "Flashlight ON (PWM #{FLASHLIGHT_ON_PWM})."
end

def flashlight_off
  puts "--- Turning flashlight OFF ---"
  cmd("DRONE DO_SET_SERVO with PARAM1 #{FLASHLIGHT_POWER_PORT}, PARAM2 #{FLASHLIGHT_OFF_PWM}")
